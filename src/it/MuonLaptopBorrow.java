...
public class MuonLaptopBorrow {
	...
	public static void main(String[] args) {
		try {
			MuonLaptopBorrow window = new MuonLaptopBorrow();
	...
	protected void createContents() {
		...
		shell.setImage(SWTResourceManager.getImage(MuonLaptopBorrow.class, "/itmanagerip/Icon/IP64.png"));
			...
			if (!textHoTen.getText().isEmpty()) {
				try {
					...
					try {
						...
						int deviceid = 0;
						String select = "SELECT TOP 1 (SELECT SUBSTRING([MaMuonThietBi],2,8)) FROM [dbo].[IT_ChoMuonThietBiLaptop] ORDER BY [MaMuonThietBi] DESC";
						...
						for (int i = 0; i < itemssave.size(); i++) {
							insert = insert
									+ "INSERT INTO [dbo].[IT_ChoMuonThietBiLaptop] ([MaMuonThietBi],[SoThe],[HoTen],[DonVi],[MaThietBi],[GhiChu],[NguoiChoMuon],[ThoiGianMuon]) VALUES ('"
	...
	public void loadDataTable() {
		...
		try {
			...
			String select = "SELECT [MaThietBi],[TenThietBi],[GhiChu] FROM [dbo].[IT_ThietBiChoMuon] WHERE [DaMuon]=0 AND [TrangThai]=1 AND [LoaiThietBi]=6 ORDER BY [TenThietBi] ASC";
