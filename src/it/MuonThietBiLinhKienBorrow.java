...
public class MuonThietBiLinhKienBorrow {
	...
	public static void main(String[] args) {
		try {
			MuonThietBiLinhKienBorrow window = new MuonThietBiLinhKienBorrow();
	...
	protected void createContents() {
		...
		shell.setImage(SWTResourceManager.getImage(MuonThietBiLinhKienBorrow.class, "/itmanagerip/Icon/IP64.png"));
		shell.setSize(1094, 568);
		...
		table.setBounds(427, 25, 634, 472);
			...
			if (!textHoTen.getText().isEmpty()) {
				try {
					...
					try {
						...
						int deviceid = 0;
						String select = "SELECT TOP 1 (SELECT SUBSTRING([MaMuonThietBi],2,8)) FROM [dbo].[IT_ChoMuonThietBiLinhKien] ORDER BY [MaMuonThietBi] DESC";
						...
						for (int i = 0; i < itemssave.size(); i++) {
							insert = insert
									+ "INSERT INTO [dbo].[IT_ChoMuonThietBiLinhKien] ([MaMuonThietBi],[SoThe],[HoTen],[DonVi],[MaThietBi],[GhiChu],[NguoiChoMuon],[ThoiGianMuon]) VALUES ('"
	...
	public void loadDataTable() {
		...
		try {
			...
			String select = "SELECT [MaThietBi],[TenThietBi],[GhiChu] FROM [dbo].[IT_ThietBiChoMuon] WHERE [TrangThai]=1 AND [LoaiThietBi]=0 ORDER BY [TenThietBi] ASC";
