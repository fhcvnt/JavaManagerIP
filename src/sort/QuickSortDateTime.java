...
public class QuickSortDateTime {
	public static void main(String[] args) {
		int i;
		String[] arr = { "24/06/2021 16:23:49", "15/02/2021 18:20:19", "02/02/2019 20:05:06", "22/03/2021 05:25:29",
	...
	public static int partition(String a[], int beg, int end) {
		...
		flag = 0;
		while (flag != 1) {
			while (compareDateTime(a[loc], a[right]) <= 0 && (loc != right))
				right--;
			if (loc == right)
				flag = 1;
			else if (compareDateTime(a[loc], a[right]) > 0) {
				temp = a[loc];
				a[loc] = a[right];
				a[right] = temp;
				loc = right;
			...
			if (flag != 1) {
				while ((compareDateTime(a[loc], a[left]) >= 0) && (loc != left))
					left++;
				if (loc == left)
					flag = 1;
				else if (compareDateTime(a[loc], a[left]) < 0) {
					temp = a[loc];
					a[loc] = a[left];
					a[left] = temp;
					loc = left;
	...
	public static void quickSort(String a[], int beg, int end) {
		int loc;
		if (beg < end) {
