...
public class EditInternet {
	...
	private String editsothe = "", edithoten = "", editdonvi = "", editip = "", editwebsite = "", editngaydenghi = "";
	...
	public static void main(String[] args) {
		try {
			EditInternet window = new EditInternet();
			window.open();
			System.exit(0);
		} catch (Exception e) {
			e.printStackTrace();
	...
	protected void createContents() {
		...
		shell.setText("Edit internet");
		shell.setImage(SWTResourceManager.getImage(EditInternet.class, "/itmanagerip/Icon/IP64.png"));
		...
		textIP.setBounds(117, 84, 180, 35);

		Button btncheckFullinternet = new Button(shell, SWT.CHECK);
		btncheckFullinternet.setFont(SWTResourceManager.getFont("Times New Roman", 18, SWT.NORMAL));
		btncheckFullinternet.setBounds(310, 84, 166, 35);
		btncheckFullinternet.setText("Full Internet");

		Button btnSave = new Button(shell, SWT.NONE);
		btnSave.setImage(SWTResourceManager.getImage(EditInternet.class, "/itmanagerip/Icon/button/save 30.png"));
		...
		btnSave.setBounds(876, 84, 130, 37);
		btnSave.setText("Save");

		CLabel lbWebsite = new CLabel(shell, SWT.NONE);
		lbWebsite.setAlignment(SWT.RIGHT);
		lbWebsite.setFont(SWTResourceManager.getFont("Times New Roman", 18, SWT.NORMAL));
		lbWebsite.setBounds(10, 291, 153, 39);
		lbWebsite.setText("Trang Web");

		textWebsite = new Text(shell, SWT.BORDER | SWT.MULTI);
		textWebsite.setBackground(SWTResourceManager.getColor(SWT.COLOR_WHITE));
		textWebsite.setFont(SWTResourceManager.getFont("Times New Roman", 18, SWT.NORMAL));
		textWebsite.setBounds(169, 139, 684, 356);
		textWebsite.setTextLimit(3000);

		CLabel lbImage = new CLabel(shell, SWT.NONE);
		lbImage.setBackground(
				SWTResourceManager.getImage(NewInternet.class, "/itmanagerip/Icon/bieutuong/network290x243.png"));
		lbImage.setBounds(876, 203, 290, 243);

		Button btnCancel = new Button(shell, SWT.NONE);
		btnCancel.setText("Cancel");
		btnCancel.setImage(SWTResourceManager.getImage(EditInternet.class, "/itmanagerip/Icon/button/cancel30.png"));
		...
		datetimeNgaydenghi.setBounds(703, 84, 150, 35);
		...
		lbNgaydenghi.setBounds(482, 84, 216, 35);
		...
		textWebsite.setText(editwebsite);
		...
		// Sự kiện chọn check box full interntet
		// ***************************************************************************************************************************
		btncheckFullinternet.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				if (btncheckFullinternet.getSelection()) {
					textWebsite.setText("Full");
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (!textSothe.getText().isEmpty() && !textHoten.getText().isEmpty() && !textIP.getText().isEmpty()
					...
					try {
						...
						try {
							String update = "UPDATE Internet SET SoThe='" + textSothe.getText() + "',Website=N'"
									+ textWebsite.getText() + "',NgayDeNghi='" + ngaydenghi
					...
					if (textSothe.getText().isEmpty()) {
						...
						thongbao.open();
					} else if (textWebsite.getText().isEmpty()) {
						MessageBox thongbao = new MessageBox(shell, SWT.ICON_ERROR | SWT.OK);
						if (language == 0) {
							thongbao.setText("Thông báo lỗi");
	...
	public void setDataInternetDefault(String sothe, String hoten, String donvi, String ip, String website,
			String ngaydenghi) {
		editsothe = sothe;
		edithoten = hoten;
		editdonvi = donvi;
